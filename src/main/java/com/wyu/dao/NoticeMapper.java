...
public interface NoticeMapper {
	...
	@Select("select count(1) from lib_notice where notice_name like #{name}")
	 ...
	 * @return
	 */
	@Select("select * from lib_notice where notice_name like #{name} order by release_time desc limit #{currentPage},#{size}")
	...
	@Select("select count(1) from lib_notice")
